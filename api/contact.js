...
function getPayload(req) {
  ...
  if (typeof b === "string") {
    ...
    return {
      ...
      to: p.get("to") || p.get("recipient") || "",
      source: p.get("source") || p.get("page") || "",
    };
  }
  return {
    name: String(b.name ?? ""),
    ...
    to: String(b.to ?? b.recipient ?? ""),
    source: String(b.source ?? b.page ?? ""),
  };
}
...
async function sendViaResend(payload) {
  ...
  const from = `${payload.name} via ${fromName} <${senderAddress}>`;
